    ...
    await db
        ...
        .then(async (resp: any) => {
            ...
            if (jobsToUpdate.length > 0) {
                await db
                    .query(`UPDATE job_orders SET Status = 'Open' WHERE job_id = ANY ($1)`, [jobsToUpdate])
                    .then((updateResp: any) => getJobOrders())
            }
...
export const setToClosed = async (jobOrderID: string) => {
    await db.query(`UPDATE job_orders SET Status = 'Closed' WHERE job_id = $1`, [jobOrderID]).catch((err: any) => {
        console.error("error while querying: ", err)
        throw new Error(err.message)
    })
}
...
export const setToOpen = async (jobOrderID: string) => {
    await db.query(`UPDATE job_orders SET Status = 'Open' WHERE job_id = $1`, [jobOrderID]).catch((err: any) => {
        console.error("error while querying: ", err)
        throw new Error(err.message)
    })
}
    ...
    await db
        .query(
          ...
          SET employer = $1,
            ...
            WHERE job_id = $7`,
            [updateBody.employer, updateBody.position, updateBody.startDate, updateBody.deadline, updateBody.catchments, updateBody.user, jobID]
        )
        .catch((err: any) => {
            console.error("error while querying: ", err)
            throw new Error(err.message)
