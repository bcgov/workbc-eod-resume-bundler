...
import generateHTMLEmail from "../utils/HtmlEmail"
...
export const getSubmissions = async (user: string, isManager: boolean, managesCatchments: string[]) => {
    ...
    if (isManager) {
        // managers can see all submissions within the catchments they manage
        queryStr += ` WHERE s.created_by = ANY ($1) OR s.catchment_id = ANY ($2)`
        queryParams = [[user], managesCatchments]
    } else if (!isManager && user != null) {
        // regular users can only see their submissions
        queryStr += ` WHERE s.created_by = ANY ($1)`
        queryParams = [[user]]
    }
    ...
    await db
        ...
        .then((resp: any) => {
            const submissions: { [id: string]: Submission } = {}
            resp.rows.forEach((a: any) => {
                ...
                if (submissions[a.submission_id] == null) {
                    // case 1: new Submission
                    const job: JobOrder = {
                        jobOrderID: a.job_id,
                        employer: a.employer,
                        position: a.position,
                    ...
                    const applicant: ClientApplication = {
                    ...
                    // case 2: Submission already exists in dictionary
                    const applicant: ClientApplication = {
                        clientApplicationID: a.client_application_id,
                        clientName: a.client_name,
                        preferredName: a.preferred_name,
            ...
            const submissionsToReturn: Submission[] = []
            Object.entries(submissions).forEach(([_, value]) => {
                submissionsToReturn.push(value)
            })
...
export const createSubmission = async (createBody: CreateSubmission, files: any) => {
    ...
    const applicants = JSON.parse(createBody.applicants.toString())
    await db
        .query(
        ...
        VALUES 
        ($1, $2, $3, $4, $5, $6, $7, $8)`,
            [submissionID, createBody.jobID, createBody.catchmentID, createBody.centreID, false, createBody.user, new Date(), createBody.email]
        )
        .then(() => {
...
export const setClientsToApproved = async (applicantIDs: string[]) => {
    await db.query(`UPDATE client_applications SET Status = 'Approved' WHERE client_application_id = ANY ($1)`, [applicantIDs]).catch((err: any) => {
        console.error("error while querying: ", err)
        throw new Error(err.message)
    })
}
...
export const setClientsToFlagged = async (applicantIDs: string[]) => {
    await db.query(`UPDATE client_applications SET Status = 'Flagged' WHERE client_application_id = ANY ($1)`, [applicantIDs]).catch((err: any) => {
        console.error("error while querying: ", err)
        throw new Error(err.message)
    })
}
...
export const setClientToDoNotBundle = async (applicantID: string) => {
    await db.query(`UPDATE client_applications SET Status = 'Do Not Bundle' WHERE client_application_id = $1`, [applicantID]).catch((err: any) => {
        console.error("error while querying: ", err)
        throw new Error(err.message)
    })
}
            ...
            .then(async (r) => {
                ...
                const message: MailOptions = {
                    ...
                    html: generateHTMLEmail(
                        "Your resume bundle from WorkBC is ready!",
                        [
                            `Hello,`,
                ...
                await db
                    .query(
                        // Set bundled statuses to true and statuses to Bundled
                        `UPDATE client_applications
             SET Bundled = true, Status = $1 WHERE client_application_id = ANY ($2)`,
                        ["Bundled", clientApplicationIDs]
                    )
                    .catch((err: any) => {
                        console.error("error while querying: ", err)
                        throw new Error(err.message)
    ...
    await db
      ...
      WHERE client_application_id = $7`,
        ...
        .then(async () => {
            ...
            const { bundle } = updateBody
            const status = updateBody.status.toLowerCase()
            if ((bundle && status !== "do not bundle") || (!bundle && status === "do not bundle")) {
                return // no need to update
            }

            if (!bundle && status !== "do not bundle") {
                const newStatus = "Do Not Bundle"
                await db
                    .query(
                        ` UPDATE client_applications
          SET status = $1
        WHERE client_application_id = $2`,
                        [newStatus, clientApplicationID]
                    )
                    .catch((err: any) => {
                        console.error("error while querying: ", err)
                        throw new Error(err.message)
...
const combinePDFBuffers = (firstBuffer: Buffer | undefined, secondBuffer: Buffer) => {
    ...
    try {
        ...
        const pdfWriter = hummus.createWriterToModify(firstPDFStream, new hummus.PDFStreamForResponse(outStream as any))
