...
const CreateJobOrderForm = () => {
    ...
    const showErrors = () => (
        <div>{/* disabling until I can figure out a way to not have the error show up randomly */}</div>
        // <div>
    ...
    return (
        <Formik
            ...
            onSubmit={(values, { setErrors, setSubmitting }) => {
                const formData = new FormData() // use form data to be able to send job description file buffer to API
                ...
                const blob = new Blob([values.jobDescription.buffer], { type: values.jobDescription.fileType }) // TODO: null check
        ...
        >
            {({ values, isSubmitting, setFieldValue, errors, hasError }) => (
                <div>
                    <Form>
                        <p>
                            Create a job order and WorkBC Centres can upload appropriate client resumes for the position. Please fill in the fields
                            below.
                        </p>
                                        ...
                                        {acceptedFiles.length === 0 && (
                                            <div {...getRootProps({ style })}>
                                                ...
                                                <p style={{ margin: "auto", paddingTop: "30px", paddingBottom: "30px", textAlign: "center" }}>
                                                    Click to upload or drag and drop the job description here
                                                </p>
                                            </div>
                                        )}
                                        {acceptedFiles.length > 0 && (
                                            <div {...getRootProps({ style })}>
                                                ...
                                                >
                                                    {acceptedFiles[0].name} uploaded. Click to re-upload a different job description
                                                </p>
                                            </div>
                                        )}
                                    </div>
                        ...
                        {catchments.length > 0 && <FastField name="catchments" component={CatchmentSelector} catchments={catchments} />}
                        <ErrorMessage name="catchments" className="field-error">
                            {(msg) => <div style={{ color: "red", weight: "bold" }}>{msg.toUpperCase()}</div>}
                        </ErrorMessage>
                        <div className="form-group">
                            <label className="col-form-label control-label" htmlFor="minimumRequirements">
                                Minimum requirements
                            </label>
                            <small className="text-muted" id="minimumRequirements">
                                {" "}
                                (1000 characters max.)
                            </small>
                            <Field
                        ...
                        <div className="form-group">
                            <label className="col-form-label control-label" htmlFor="otherInformation">
                                Other information
                            </label>
                            <small className="text-muted" id="otherInformation">
                                {" "}
                                (1000 characters max.)
                            </small>
                            <Field as="textarea" className="form-control" id="otherInformation" name="otherInformation" rows="4" maxLength="1000" />
                            <small>{values.otherInformation.length}/1000</small>
                        </div>
                        <div className="form-group">{Object.keys(errors).length >= 1 && showErrors()}</div>
                        <button
