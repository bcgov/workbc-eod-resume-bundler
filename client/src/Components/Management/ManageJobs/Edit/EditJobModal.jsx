...
const EditJobModal = ({ job, catchments, show, handleClose }) => {
    ...
    const MarkForDeleteModal = (props) => {
        ...
        return (
            <Modal show={showMarkForDelete} onHide={handleMarkForDeleteClose(jobID)}>
                <Modal.Header>
                ...
                <Modal.Body>
                    <h5 style={{ color: "grey", fontWeight: "lighter" }}>Type the ID of the application to confirm mark for deletion:</h5>
                    <Formik initialValues={props} enableReinitialize>
                        <Form>
                            <div>
                                <div className="form-group col-md-6">
                                    <label className="control-label" htmlFor="app-id">
                                        Confirm ID
                                    </label>
                                    <Field name="app-id" type="text" className="form-control" />
                                    <ErrorMessage name="app-id" component="div" className="field-error" />
                                </div>
                            </div>
                        </Form>
                    </Formik>
                ...
                <Modal.Footer>
                    <button className="btn btn-danger" type="button" onClick={handleMarkForDeleteClose(jobID)}>
                        Mark for Delete
                    </button>
                    <button className="btn btn-outline-primary" type="button" onClick={handleMarkForDeleteClose(jobID)}>
                        Cancel
                    </button>
                </Modal.Footer>
            </Modal>
    ...
    return (
        <Modal show={show[job.id]} onHide={handleClose(job.id, false)} size="xl">
                ...
                >
                    {({ isSubmitting }) => (
                        <Form>
                            ...
                            {catchments.length > 0 && (
                                <FastField name="catchments" component={CatchmentSelector} catchments={catchments} initialSelected={job.catchments} />
                            )}
            ...
            <Modal.Footer />
            <MarkForDeleteModal jobID={job.id} />
        </Modal>
    )
}
